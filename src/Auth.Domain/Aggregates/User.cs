...
public class User : AggregateRoot, IEntityBase<long>
{
    ...
    public User(long id, string email, string securityStamp, string refreshToken, DateTime refreshTokenExpiryTime)
    {
        Id = id;
        Email = email;
    ...
    public void RefreshSecurityParams(int refreshExpiryMinutes = 3600)
    {
        ...
        var @event = new UserLoginEvent(this);
        AddEvent(@event);
    }
    ...
    public void Logout()
    {
        ...
        DeviceType = null;
        DeviceToken = null;
        var @event = new UserLogoutEvent(this);
        AddEvent(@event);
    }
}