...
public class StaffRegistrationStep3Handler : IRequestHandler<StaffRegistrationStep3Command, OneOf<bool,Failure>>
{
    private readonly IUnitOfWork _unitOfWork;
    private readonly IJwtTokenGenerator _jwtTokenGenerator;

    public StaffRegistrationStep3Handler(IUnitOfWork unitOfWork, IJwtTokenGenerator jwtTokenGenerator)
    {
        _unitOfWork = unitOfWork;
        _jwtTokenGenerator = jwtTokenGenerator;
    }
    
    public async Task<OneOf<bool,Failure>> Handle(StaffRegistrationStep3Command request, CancellationToken cancellationToken)
    {
        // ----------------------------------------------------------------------------------------------------
        ...
        // Processing - 如果還沒準備好要註冊, 翻活錯誤結果
        if (task.IsT1) return task.AsT1;
        
        // Processing - 註冊
        registration.RegisterForStaff(request.Password);
