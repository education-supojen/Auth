...
public class UserInformationQueryHandler : IRequestHandler<UserInformationQuery, OneOf<UserInformationDto,Failure>>
{
    private IUnitOfWork _unitOfWork;
    private IJwtTokenGenerator _jwtTokenGenerator;
    private IMapper _mapper;

    public UserInformationQueryHandler(IUnitOfWork unitOfWork, IJwtTokenGenerator jwtTokenGenerator, IMapper mapper)
    {
    ...
    public async Task<OneOf<UserInformationDto,Failure>> Handle(UserInformationQuery request, CancellationToken cancellationToken)
    {
        // ----------------------------------------------------------------------------------------------------
        // Begin Transaction 
        // ----------------------------------------------------------------------------------------------------
        ...
        var user = await _unitOfWork.UserRepository.CacheGetAsync(id);

        // Processing - 如果使用者為空, 代表無法認證身份
        if (user == null) return Failures.Token.TokenInvalid;

        // Processing - 如果 securitystamp 不對, 代表無法認證身份
        if (user.SecurityStamp != tokenVariables.AsT0.jti) return Failures.Token.TokenInvalid;
        ...
        // Processing - 回傳使用者資訊
        return _mapper.Map<UserInformationDto>(user);
    }
}